...
const Home = () => {
    const {state:{products},filterState,user,dispatch} = CartContext();
    const {ascending,discending,fastDelivery,stars,query} = filterState
    let productsDummy = []
  if(query!==''){
    productsDummy=productsDummy.length===0?products.filter(
      (pr)=>pr.Name.toLowerCase().includes(query.toLowerCase())):
      productsDummy.filter((pr)=>pr.Name.toLowerCase().includes(query.toLowerCase()))
      console.log(query)
  }
  ...
  if(fastDelivery){
   productsDummy=productsDummy.length===0?products.filter((pr)=>pr.delivery===1):
   productsDummy.filter((pr)=>pr.delivery===1)
  }
  if(stars!==0){
